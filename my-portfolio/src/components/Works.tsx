  ...
  return (
    <article className="work-card">
      <div className="work-thumbnail">
        <SafeImg src={work.thumbnail} alt={work.title} fallback="" />
      </div>
        ...
        <div className="work-body">
            ...
            {work.venues?.length > 0 && (
                <div className="work-venues">
                    {work.venues.map((v,i)=>(
                        <span key={i} className="work-venues-tag">{v.name} - {v.date}</span>
                    ))}
                </div>
            )}
            ...
            <div className="work-links">
                ...
                <a href={work.github} className="btn btn-outline work-btn"
                 target="_blink" rel="noreferrer">Github</a>
                 {work.link && (
                    <a href={work.link} className="btn btn-outline work-btn"
                    target="_blank" rel="noreferrer">LiveDemo</a>
                 )}
            </div>
        </div>
    </article>
